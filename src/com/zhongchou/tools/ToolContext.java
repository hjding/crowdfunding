...
public class ToolContext {
	...
	public static User getCurrentUser(HttpServletRequest request,
		...
		if (null != loginCookie && !loginCookie.equals("")) {
			...
			// PropertiesPlugin.getParamMapValue(DictKeys.config_securityKey_key);
			...
			int day = ToolDateTime.getDateDaySpace(start,
	...
	public static void setCurrentUser(HttpServletRequest request,
			HttpServletResponse response, User user, boolean autoLogin) {
		// 1.设置cookie有效时间
		int maxAgeTemp = -1;
		if (autoLogin) {
			// maxAgeTemp = ((Integer)
			// PropertiesPlugin.getParamMapValue(DictKeys.config_maxAge_key)).intValue();
			maxAgeTemp = 7 * 24 * 3600 * 1000;// 7天有效cookie
		}

		// 2.设置用户名到cookie
		ToolWeb.addCookie(response, "", "/", true, "userName",
				user.getStr("name"), maxAgeTemp);
		...
		token.append(date).append(".#.").append(userIds).append(".#.")
				.append(ips).append(".#.").append(userAgent);
		...
		// 6. 添加到Cookie
		ToolWeb.addCookie(response, "", "/", true, "zhongchou.authmark",
				securityCookie, maxAgeTemp);
	}
}
