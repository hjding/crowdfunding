...
public class UpFileRenamePolicy implements FileRenamePolicy {
	...
	public File rename(File f) {
		...
		relative_directory = File.separator + (StrKit.isBlank(saveDir) ? "upload" : saveDir) + File.separator
				+ dateFormat.format(new Date()) + File.separator + System.currentTimeMillis() + getFileExt(f.getName());
		StringBuilder newFileName = new StringBuilder(webRoot).append(relative_directory);
